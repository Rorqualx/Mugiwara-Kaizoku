...
### Update Download Client Paths

After mounting, configure your download clients to report the mounted path:
...
### Path Mapping (Alternative)

If you can't change download client paths, the app includes automatic path mapping:
...
export PATH_MAPPINGS="/data/downloads:/mnt/nas-downloads,/volume1/downloads:/Volumes/nas-downloads"
```

Add to `.env` file:
```env
PATH_MAPPINGS=/data/downloads:/mnt/nas-downloads
```

#### Option 2: Programmatic Configuration
The app automatically loads path mappings on startup. You can also configure them dynamically:

```typescript
import { getPathMapper } from './src/server/services/download/pathMapper';

const pathMapper = getPathMapper();
pathMapper.addMapping({
  remotePath: '/data/downloads',
  localPath: '/mnt/nas-downloads',
  description: 'NAS downloads folder'
});
```

#### How Path Mapping Works

1. Download client reports: `/data/downloads/manga.cbz`
2. Path mapper detects prefix `/data/downloads`
3. Replaces with local mount: `/mnt/nas-downloads/manga.cbz`
4. File import uses mapped path

**Benefits:**
- No need to reconfigure download clients
- Supports multiple path mappings
- Works with any download client
- Can map different remote servers to different local mounts

**Example Mappings:**
```env
# Map multiple NAS paths
PATH_MAPPINGS=/data/torrents:/mnt/nas1/torrents,/data/usenet:/mnt/nas2/usenet

# Map Windows UNC paths
PATH_MAPPINGS=//nas-server/downloads:Z:/downloads
