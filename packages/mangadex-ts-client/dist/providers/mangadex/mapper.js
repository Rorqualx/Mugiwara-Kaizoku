...
exports.mapMangaDexManga = mapMangaDexManga;
exports.mapMangaDexChapter = mapMangaDexChapter;
/** Map MangaDex manga to UnifiedManga */
function mapMangaDexManga(manga, client, options) {
    ...
    for (const rel of authorRels) {
        ...
        creators.push({
            id: rel.id,
            name: authorData?.attributes.name || rel.attributes?.['name'] || 'Unknown',
            role: 'AUTHOR',
...
function mapMangaDexChapter(chapter) {
...
function mapCovers(mangaId, relationships, client, allCovers) {
...
function mapVolumesFromAggregate(mangaId, aggregate, covers, client) {
    if (!aggregate)
        return [];
    const volumes = [];
