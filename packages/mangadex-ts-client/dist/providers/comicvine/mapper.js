...
exports.mapComicVineVolume = mapComicVineVolume;
const scraper_1 = require("./scraper");
/** Map ComicVine person role string to CreatorRole */
function mapCreatorRole(role) {
    if (!role)
        return 'OTHER';
    const r = role.toLowerCase();
    if (r.includes('writ') || r.includes('story') || r.includes('author'))
        return 'AUTHOR';
    if (r.includes('art') || r.includes('illustrat') || r.includes('pencil') || r.includes('draw'))
        return 'ARTIST';
    if (r.includes('ink'))
        return 'INKER';
    if (r.includes('color'))
...
function mapComicVineVolume(volume, issues) {
    ...
    const description = {};
    if (volume.deck)
        description['en'] = volume.deck;
    else if (volume.description)
        description['en'] = (0, scraper_1.cleanDescription)(volume.description);
    // Cover images
    const covers = {
    ...
    // Story arcs
    const storyArcs = volume.story_arcs?.map((sa) => ({
        id: String(sa.id),
        name: sa.name,
        source: 'comicvine',
    })) || [];
...
function mapIssuesToVolumes(issues) {
    return issues.map((issue) => {
        const chapterRange = issue.description
            ? (0, scraper_1.extractChapterRange)(issue.description)
            : undefined;
        ...
        return {
            ...
            title: issue.name,
            description: issue.deck || (issue.description ? (0, scraper_1.cleanDescription)(issue.description) : undefined),
...
function mapIssueToChapter(issue) {
    return {
        id: String(issue.id),
...
function deriveStatus(volume, issues) {
