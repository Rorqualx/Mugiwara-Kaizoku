...
export const FANDOM_SELECTORS: SourceSelectors = {
  ...
  altTitles: [
    ...
    // Chapter page specific - Kana and Rōmaji fields
    '.pi-data[data-source="kana"] .pi-data-value',
    '.pi-data[data-source="rōmaji"] .pi-data-value',
    ...
    '.pi-data .pi-data-label:contains("Rōmaji") + .pi-data-value',
  ...
  summary: [
    ...
    // Fandom uses <h2><span id="Synopsis">...</span></h2> structure
    'h2:has(#Synopsis) + p',
    'h2:has(#Plot) + p',
    'h2:has(#Summary) + p',
    'h2:has(#Story) + p',
  ...
  characters: [
    ...
    '.pi-data[data-source="character"] .pi-data-value a',  // Singular form for Volume pages
    '.pi-data[data-source="main_characters"] .pi-data-value a',
    // Also extract character names as text when not linked
    '.pi-data[data-source="characters"] .pi-data-value',
    '.pi-data[data-source="character"] .pi-data-value',
    // Removed: 'a[href*="/Characters"]' - too broad, matches sidebar navigation links
