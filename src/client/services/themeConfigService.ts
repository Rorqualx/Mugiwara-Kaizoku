...
class ThemeConfigService {
  ...
  /**
   * Type guard to validate ThemeConfig structure
   *
   * @param value - Value to validate
   * @returns True if value is a valid ThemeConfig
   */
  private isValidThemeConfig(value: unknown): value is ThemeConfig {
    if (typeof value !== 'object' || value === null) return false;

    const config = value as Record<string, unknown>;

    // Check documentation
    if (typeof config['documentation'] !== 'object' || config['documentation'] === null) return false;
  ...
  async loadConfig(): Promise<ThemeConfig> {
    try {
      ...
      logger.warn('Invalid theme configuration structure, using defaults');
...
logger.error(`Failed to load theme configuration: ${errorMessage}`);
  ...
  async saveConfig(config: ThemeConfig): Promise<void> {
    try {
      await fs.writeFile(
        this.configPath, JSON.stringify(config, null, 2),
        'utf-8'
      );
    } catch (error: unknown) {const errorMessage = error instanceof Error ? error.message : String(error);
logger.error(`Failed to save theme configuration: ${errorMessage}`);
      throw new Error(`Failed to save theme configuration: ${error}`);
    }
  }
